...
- **Theme & fonts**: CSS vars on `:root` + `.dark-mode`. `--font-head:'Baloo 2'` (headings), `--font-body:'Nunito'` (body), loaded from Google Fonts. **Luckiest Guy is reserved for the hub "ClassTools" logo only** (`--font-logo` in `Toolbox/index.html`, applied to `.topbar .brand` + `.hub-header h1`); do NOT use it elsewhere — it reads as wacky at body sizes. To re-skin, change the two `--font-*` vars + the Google Fonts `<link>` (kept per-file). Primary navy `#25426B`/`#3D6396` (aligned to the Maestr site brand 2026-07; dark mode `#6C92C7`/`#8FB0DC`). Palette `PAL`/`PALETTE` first entry is `#3D6396`; the rest of the rainbow (`#ff3d7f`,`#14d6c4`,`#ffb020`,…) is game color-coding, not brand — leave it. In-game `--gold` stays `#ffb020` (reward signaling).
